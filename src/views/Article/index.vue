...
// 因为只有这块才需要用这个样式来渲染 所以不用写在main.js里面
import 'github-markdown-css'
import { getArticle } from '@/api/article'
export default {
  name: 'ArticleIndex',
  components: { FollowUser, CollectArticle, ArticleComment, AddComment, ReplayComment },
  props: {
    article_id: {
      type: [Number, String],
      required: true
  ...
  async created () {
    try {
      ...
      // 可选连写法err.response?.status
      if (err.response && err.response?.status === 404) {
        this.is404Error = true
      }
    }
    this.isLoading = false
    // 获取正文中所有img图片 nextTick等数据视图加载完成之后才执行，如果写在别的地方 数据没有加载完成就只能得到一个空数组
    this.$nextTick(() => {
      const arr = document.querySelectorAll('.article-content img')
      if (arr.length === 0) return // 防御性代码 没有图片下面就不执行
      const arr1 = []
